...
loop(Socket, Subscribers, Users) ->
    receive %%crazy levels of pattern matching
        {register, Pid, Usr} ->
            ...
            loop(Socket, Subscribers, [Usr|Users]);                       %% adding the the new Usr to the list of Users registered
        {tcp, Socket, Bin} ->                                             %% message received from the network, nothing actors that send this message, Bin is the HTTP request
            ...
            case parse_response(Bin) of                             
                ...
                {post, "update", ["topic", TopicS, "msg", Msg]} ->
                    ...
                    [spawn(fun () -> forward(Sub, Msg, Socket) end)|| Sub <- Subs], %%list comprehension for spawning more than one proc, one proc for each subs, the idea : i update a new topic and forward the update to all the subs
                    loop(Socket, Subscribers, Users);
                {post, "register", ["host",Host,"port",PortS, "topic", TopicS]} ->
                    ...
                    #{Topic := Subs} = Subscribers,                                 %% Map
                    spawn(fun () -> respond(Host, Port, Socket) end),               %% i need to handle the request with a new proc, 
...
respond(Host, Port, ResponseSocket) ->                                          %% need to handle the request from a client                            
    {ok, RequestSocket} = gen_tcp:connect(Host, Port, [binary, {packet, 0}]),
    ...
    receive
        {tcp, RequestSocket, Bin} ->
            ...
            server ! {register, self(), binary_to_list(Bin)}, %% intra process communication, this is a process in the server that communicates with an other process server of the server
...
get_uri_params(get, Text,_) -> ((Text -- " GET ") -- " HTTP/1.1 ") -- "/";  %%  -- takes out of the left arg the first occurance of the right arg 
...
parse_response(Bin) ->
    [Text|Rest] = string:tokens(binary_to_list(Bin), "\r\n"), %% First line is the GET request, i tokenize the binary_to_list(Bin) 
    Method =response_method(Text),                            %% Pattern match on the ASCII to retrieve the method invoked by the request
